...
- 加载受支持地图的 CS2 NAV 数据，并限制编辑内容落在可达表面。
- 可选加载本地 GLB 地图模型，并调节模型透明度。
...
GLB 地图模型不会提交到仓库。若需要显示地图几何体，请将模型放在 `public/maps/<map>/<map>.glb`。
