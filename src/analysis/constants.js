// Shared analysis option values, labels, colors, and display metadata.
export const ANALYSIS_AREA_PHASES = ['early', 'mid', 'post'];

export const ANALYSIS_UTILITY_KINDS = ['smoke', 'flash', 'fire', 'he', 'decoy'];

export const ANALYSIS_UTILITY_COLORS = {
  smoke: '#9aa6a0',
  flash: '#f4e99b',
  fire: '#ff6b47',
  he: '#83c56b',
  decoy: '#9b8cff',
};

export const ANALYSIS_UTILITY_ICONS = {
  smoke: 'smoke',
  flash: 'flash',
  fire: 'molotov',
  he: 'grenade',
  decoy: 'decoy',
};

export const ECONOMY_CATEGORIES = ['PISTOL', 'ECO', 'HALF', 'FORCE', 'FULL'];

export const HEAT_HEIGHT_SLICES = 64;
export const HEAT_ATLAS_GRID = 8;
export const HEAT_SLICE_SIZE = 192;
