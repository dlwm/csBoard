...
export default function useRadarOverlay({ activePanel, boardRef, mapName, navData }) {
  ...
  return useMemo(() => {
    ...
    const square = squareRadarBounds(bounds.minZ, bounds.maxZ, bounds.minX, bounds.maxX);
    const project = (position, clamp = true) => {
      const rawX = 4 + (position[2] - square.minX) / square.size * 92;
      const rawY = 96 - (position[0] - square.minY) / square.size * 92;
