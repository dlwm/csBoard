...
export const normalizeCollabWorkspace = (workspace) => {
  ...
  // Older anonymous-frame saves duplicated full trajectories and smoke journals
  // inside sourceNote; normalize them into the compact single-copy schema.
  const collabUtilities = (frame.collabUtilities || []).map((item, index) => compactAnonymousUtility({ ...item, id: item.id || `imported-${item.noteId || index}` }));
