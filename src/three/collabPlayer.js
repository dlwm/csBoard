...
export function createCollabPlayer({ position, id, name, team = 'T', crouched = false, pitch = 0 }) {
  ...
  rifle.userData.collabWeapon = 'rifle';
  ...
  const aimRay = new THREE.Line(
    new THREE.BufferGeometry().setAttribute('position', new THREE.Float32BufferAttribute([0, 0.15, 0, 0, 0.15, -1], 3)),
...
export function updateCollabPlayerAim(group, collisionMeshes, raycaster, collisionVersion) {
  ...
  const origin = new THREE.Vector3(0, aimRay.position.y, -0.42);
  const pitch = group.userData.collabPitch || 0;
  const placeTarget = (length) => {
    aimRay.scale.z = length;
    aimRay.rotation.x = -pitch;
    aimTarget.position.copy(new THREE.Vector3(0, 0.15, -length).applyEuler(aimRay.rotation).add(origin));
  };
  ...
  const lineOrigin = new THREE.Vector3(0, 0.15, 0).add(origin);
  const worldOrigin = group.localToWorld(lineOrigin.clone());
  ...
  raycaster.near = 0.05;
  raycaster.far = 72;
  const worldLength = Math.max(0.05, (raycaster.intersectObjects(collisionMeshes, false)[0]?.distance ?? 72) - 0.03);
