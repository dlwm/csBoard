...
function updateFloorFadeState(state, mapName, floor, modelCenterY = 0, navData = null) {
  ...
  const heights = Object.values(navData.areas).flatMap((area) => area.corners.map((point) => Number(point.z)).filter(Number.isFinite));
  const selected = boundary == null ? heights : heights.filter((height) => floor === 'lower' ? height <= boundary : height >= boundary);
  if (!selected.length) {
    state.set(0, 0, 0, 0);
    return;
  }
  const bottom = boundary != null && floor === 'main' ? boundary : Math.min(...selected) - FLOOR_BOTTOM_PADDING;
  const top = boundary != null && floor === 'lower' ? boundary : Math.max(...selected) + FLOOR_TOP_PADDING;
  state.set(bottom * MAP_UNITS_TO_METERS - modelCenterY, top * MAP_UNITS_TO_METERS - modelCenterY, 0, 1);
}
