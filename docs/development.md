...
以下记录来自功能迭代和故障修复，最近核对于 2026-09-17。它不是未来功能清单；路径、常量和实现仍以当前代码为准。修改已确认行为前，说明原因及影响，不要把旧设计当作待修复问题。
...
- 当前烟雾是连续、不透明的平滑体积，重点是看清覆盖范围，不是彼此分离的小球。`SMOKE_VOLUME_SCALE` 当前为 **1.1**；此前的 1.5、1.25、1.125 是历史试调，不是当前需求。回合浏览、道具速记和协作导入应保持相同基准。
